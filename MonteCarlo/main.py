...
def user_mode():
    ...
    train_episodes = int(input("Nombre d'épisodes d'entraînement [50000] : ") or 50000)
    test_episodes = int(input("Nombre d'épisodes de test [25] : ") or 25)
    alpha = float(input("Learning rate α [0.05] : ") or 0.05)
    gamma = float(input("Discount factor γ [0.95] : ") or 0.95)
    epsilon = float(input("Epsilon initial [1.0] : ") or 1.0)
    epsilon_min = float(input("Epsilon minimum [0.01] : ") or 0.01)
    epsilon_decay = float(input("Epsilon decay [0.9997] : ") or 0.9997)
